...
import {ETHToWrappedLSTSwapHookBaseTest} from "test/fork/ETHToWrappedLSTSwapHookBaseTest.t.sol";
import {ETHToWstETHSwapHook, IWstETH} from "src/periphery/ETHToWstETHSwapHook.sol";
...
/// @notice Fork test for ETHToWstETHSwapHook.
///
/// Vault pool: stETH SmoothYieldVault wrapper ↔ Euler WETH vault wrapper
/// Asset pool: ETH ↔ wstETH (initialised inside the hook constructor)
contract ETHToWstETHSwapHookTest is ETHToWrappedLSTSwapHookBaseTest {
    ...
    function _dealDustToHook() internal override {
        // Absorb Lido/WETH rounding in the hook.
    ...
    function test_hookImmutables() public view {
        ...
        assertEq(hook.WRAPPED_LST(), WST_ETH);
        assertEq(hook.REBASING_LST(), ST_ETH);
        assertEq(hook.WETH(), WETH_ADDR);
    }

    function test_rebalance_wstETH_specific() public {
        vm.deal(address(this), 1 ether);
        hook.addWarmLiquidityETH{value: 0.5 ether}();
        _dealWstETH(address(this), 0.5 ether);
        IERC20(WST_ETH).approve(address(hook), 0.5 ether);
        hook.addWarmLiquidityLST(0.5 ether);

        // Skew LST side by swapping LST→ETH repeatedly
        ...
        wstETHHook.rebalanceWarmLiquidity();
        ...
        assertLt(diffAfter, diffBefore, "wstETH rebalance should move closer to 50/50");
    }
