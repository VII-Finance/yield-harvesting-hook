...
import {ERC4626VaultWrapperHook} from "src/ERC4626VaultWrapperHook.sol";
...
contract ERC4626VaultWrapperHookTest is ERC4626Test {
    ...
    function setUp() public override {
        ...
        deployCodeTo(
            "ERC4626VaultWrapperHook",
            abi.encode(IPoolManager(poolManager), underlyingVault, harvester, "Vault Wrapper", "VW"),
            _vault_
        );
    ...
    function setUpVault(Init memory init) public override {
        ...
        for (uint256 i = 0; i < N; i++) {
            ...
            ERC4626VaultWrapperHook(_vault_).deposit(underlyingVaultSharesMinted, user);
            vm.stopPrank();
    ...
    function setUpYield(Init memory init) public override {
        if (init.yield >= 0) {
            ...
            try underlyingAsset.mint(address(underlyingVault), gain) {
                ...
                vm.prank(harvester);
                ERC4626VaultWrapperHook(_vault_).harvest(harvestReceiver);

                uint256 profitForHarvester = FullMath.mulDiv(
                    ERC4626VaultWrapperHook(_vault_).totalAssets(),
                    gain,
                    ERC4626(address(underlyingVault)).totalSupply()
                );
